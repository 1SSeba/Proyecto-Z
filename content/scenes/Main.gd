...
func _setup_scene_structure():
	...
	if not debug_console:
		var debug_scene = preload("res://content/scenes/Debug/DebugConsole.tscn")
		if debug_scene:
			debug_console = debug_scene.instantiate()
...
func _connect_game_state_signals():
	...
	if not GameStateManager.state_changed.is_connected(_on_game_state_changed):
	...
	if not GameStateManager.game_paused.is_connected(_on_game_paused):
		GameStateManager.game_paused.connect(_on_game_paused)
	
	if not GameStateManager.game_resumed.is_connected(_on_game_resumed):
		GameStateManager.game_resumed.connect(_on_game_resumed)
	
	if not GameStateManager.player_died.is_connected(_on_player_died):
...
func _spawn_player():
	...
	var player_scene = preload("res://content/scenes/Characters/Player/Player.tscn")
	if not player_scene:
