...
var cache_access_count: Dictionary = {}

# Cache de assets más usados
...
func _ensure_audio_buses_exist():
	"""Verifica que los buses de audio existan y los crea si es necesario"""
	print("AudioManager: Verifying audio buses...")
	
	# Verificar bus Master (siempre existe)
	var master_index = AudioServer.get_bus_index("Master")
	if master_index == -1:
		print("AudioManager: ERROR - Master bus not found!")
		return
	
	# Verificar o crear bus Music
	var music_index = AudioServer.get_bus_index(MUSIC_BUS)
	if music_index == -1:
		print("AudioManager: Creating Music bus...")
		AudioServer.add_bus()
		var new_music_index = AudioServer.get_bus_count() - 1
		AudioServer.set_bus_name(new_music_index, MUSIC_BUS)
		AudioServer.set_bus_send(new_music_index, "Master")
		print("AudioManager: Music bus created at index %d" % new_music_index)
	else:
		print("AudioManager: Music bus found at index %d" % music_index)
	
	# Verificar o crear bus SFX
	var sfx_index = AudioServer.get_bus_index(SFX_BUS)
	if sfx_index == -1:
		print("AudioManager: Creating SFX bus...")
		AudioServer.add_bus()
		var new_sfx_index = AudioServer.get_bus_count() - 1
		AudioServer.set_bus_name(new_sfx_index, SFX_BUS)
		AudioServer.set_bus_send(new_sfx_index, "Master")
		print("AudioManager: SFX bus created at index %d" % new_sfx_index)
	else:
		print("AudioManager: SFX bus found at index %d" % sfx_index)
	
	print("AudioManager: Audio buses verification complete")
...
func _finalize_initialization():
	...
	# Inicializar cache de assets
	_initialize_asset_cache()
	
	is_initialized = true
	audio_manager_ready.emit()
	print("AudioManager: Initialization complete with lazy loading")

# =======================
#  LAZY LOADING OPTIMIZADO
# =======================

func _initialize_asset_cache():
	"""Inicializa el sistema de cache de assets"""
	asset_cache.clear()
	cache_access_count.clear()
	print("AudioManager: Asset cache initialized")

func get_audio_asset_cached(path: String) -> AudioStream:
	"""Obtiene un asset de audio con lazy loading y cache"""
	# Incrementar contador de acceso
	cache_access_count[path] = cache_access_count.get(path, 0) + 1
	
	# Check cache first
	if asset_cache.has(path):
		return asset_cache[path]
	
	# Cache miss - cargar asset
	var asset = load_audio_asset_lazy(path)
	if asset:
		_store_in_asset_cache(path, asset)
	
	return asset

func load_audio_asset_lazy(path: String) -> AudioStream:
	"""Carga un asset de audio de forma perezosa"""
	if not ResourceLoader.exists(path):
		push_warning("AudioManager: Asset not found: %s" % path)
		return null
	
	var asset = load(path) as AudioStream
	if asset:
		print("AudioManager: Loaded asset: %s" % path)
	else:
		push_warning("AudioManager: Failed to load audio asset: %s" % path)
	
	return asset

func _store_in_asset_cache(path: String, asset: AudioStream):
	"""Almacena un asset en cache con gestión de límites"""
...
func _cleanup_asset_cache():
	"""Limpia el cache de assets usando LRU"""
	# Encontrar el asset menos usado
	var min_access = 999999
	var least_used_path = ""
	
	for path in cache_access_count.keys():
		if cache_access_count[path] < min_access:
			min_access = cache_access_count[path]
			least_used_path = path
	
	# Remover el menos usado
	if least_used_path != "":
		asset_cache.erase(least_used_path)
		cache_access_count.erase(least_used_path)
		print("AudioManager: Removed from cache: %s (access count: %d)" % [least_used_path, min_access])

func preload_critical_assets():
	"""Pre-carga assets críticos de forma asíncrona"""
	var critical_assets = [
		"res://audio/ui/click.ogg",
		"res://audio/ui/hover.ogg",
		"res://audio/player/hurt.ogg"
	]
	
	for path in critical_assets:
		if ResourceLoader.exists(path):
			get_audio_asset_cached(path)

func get_cache_stats() -> Dictionary:
	"""Retorna estadísticas del cache de assets"""
	return {
		"cache_size": asset_cache.size(),
		"max_cache_size": max_cache_size,
		"cached_assets": asset_cache.keys(),
		"access_counts": cache_access_count.duplicate()
	}
...
func play_sfx_at_position(sound: AudioStream, position: Vector2, volume: float = 0.0, max_distance: float = 2000.0) -> AudioStreamPlayer2D:
	...
	if current_scene:
		current_scene.add_child(positional_player)
		positional_player.play()
	else:
		print("⚠️ AudioManager: No current scene for positional audio")
		return
