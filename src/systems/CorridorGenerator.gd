...
func _calculate_connection_weight(room_a, room_b) -> float:
	...
	if room_a.type == DungeonWorld.RoomType.START or room_b.type == DungeonWorld.RoomType.START:
		weight *= 0.8  # Reducir peso para priorizar
	
	# Penalizar conexiones con habitaciones especiales
	if room_a.type == DungeonWorld.RoomType.BOSS or room_b.type == DungeonWorld.RoomType.BOSS:
		weight *= 1.5  # Aumentar peso para que sea menos probable
	
	if room_a.type == DungeonWorld.RoomType.SECRET or room_b.type == DungeonWorld.RoomType.SECRET:
		weight *= 1.3
...
func _generate_corridor_between_rooms(room_a, room_b, tilemap):
	...
	# Crear corredor
	var corridor = DungeonWorld.Corridor.new()
...
func _create_doors(corridor, room_a, room_b):
	...
	var door_a = DungeonWorld.Door.new(door_a_id, corridor.path[0], Vector2i.ZERO)
	...
	var door_b = DungeonWorld.Door.new(door_b_id, corridor.path[-1], Vector2i.ZERO)
