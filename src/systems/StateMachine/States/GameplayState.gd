...
func _ensure_main_scene():
	...
	if not current_scene or current_scene.scene_file_path != "res://content/scenes/Main.tscn":
		print("GameplayState: Loading Main scene...")
		get_tree().change_scene_to_file("res://content/scenes/Main.tscn")
		
		# Esperar a que la nueva escena esté lista de forma segura
		await get_tree().process_frame
		current_scene = get_tree().current_scene
		while not current_scene:
			await get_tree().process_frame
			current_scene = get_tree().current_scene
		
		# Ahora esperar a que esté completamente ready
		if current_scene.has_signal("ready") and not current_scene.is_node_ready():
