...
@onready var debug_keys: VBoxContainer = $ConsolePanel/Margin/Content/DebugKeys
...
func _setup_console_controls() -> void:
	...
	if command_input:
		...
		command_input.placeholder_text = command_input.placeholder_text if command_input.placeholder_text != "" else "Ej: damage 10"
	if execute_button:
...
func _scroll_console_to_bottom() -> void:
	...
	if v_bar:
		console_scroll.scroll_vertical = int(v_bar.max_value)
...
func _on_pause_menu_settings_requested() -> void:
	add_log("Configuración (en pausa) aún no implementada")
