...
func _setup_room_generator():
	...
	room_generator = RoomsSystem.new() if typeof(RoomsSystem) != TYPE_NIL else null
	if room_generator:
		room_generator._seed = actual_seed
		# Configurar el tamaño si la propiedad existe
		if room_generator.has_method("set_dungeon_size"):
			room_generator.set_dungeon_size(dungeon_size)
...
func _place_special_elements():
	...
	var rooms = current_dungeon_data.rooms as Array[RoomsSystem.Room]
...
func _place_player_spawn(room: RoomsSystem.Room):
...
func _place_treasure_elements(room: RoomsSystem.Room):
...
func _place_boss_elements(room: RoomsSystem.Room):
...
func _place_secret_elements(room: RoomsSystem.Room):
	"""Coloca elementos de room secreta"""
	# Tesoro especial
	var treasure_pos = Vector2i(room.center.x - 1, room.center.y)
	tilemap_layer.set_cell(treasure_pos, TileType.TREASURE, Vector2i(0, 0))

func _place_normal_elements(room: RoomsSystem.Room):
...
func _place_room_enemies(room: RoomsSystem.Room, count: int):
...
# =======================
func get_room_at_position(world_pos: Vector2i) -> RoomsSystem.Room:
