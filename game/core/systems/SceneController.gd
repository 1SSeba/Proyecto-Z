...
func _ready():
	_logger = LoggableBehavior.new("SceneController")
...
func change_scene(scene_path: String):
	...
	if is_transitioning:
		_logger.log_warn("Already transitioning, ignoring request")
		return

	if not ResourceLoader.exists(scene_path):
		_logger.log_error("Scene does not exist: %s" % scene_path)
	...
	_logger.log_info("Changing to scene: %s" % scene_path)
	...
	if result == OK:
		...
		_logger.log_info("Scene changed successfully")
	else:
		scene_change_failed.emit(scene_path, "Failed to load scene")
		_logger.log_error("Failed to change scene: %s" % result)
