# Sistema de World y Rooms para Roguelike
class_name DungeonWorld
extends Node2D
# DungeonWorld.gd - Sistema de mazmorras
...
# =======================
func _ready():
	print("DungeonWorld: Initializing professional room system...")
...
func _setup_components():
	...
	if not tilemap_layer:
		push_error("DungeonWorld: TileMapLayer not found!")
		return
	
	tilemap = tilemap_layer
	print("DungeonWorld: Components initialized")
...
func generate_new_dungeon(p_seed: int = 0) -> bool:
	...
	print("DungeonWorld: Generating dungeon with seed %d" % generation_seed)
	...
	if not _generate_rooms(rng):
		print("DungeonWorld: Failed to generate rooms")
		return false
	
	# Conectar habitaciones
	if not _connect_rooms(rng):
		print("DungeonWorld: Failed to connect rooms")
		return false
	
	# Generar geometría
	if not _generate_dungeon_geometry():
		print("DungeonWorld: Failed to generate geometry")
		return false
	...
	print("DungeonWorld: Dungeon generated successfully with %d rooms and %d corridors" % [current_dungeon.rooms.size(), corridors.size()])
	return true
...
func _create_room(id: int, type: RoomType, preferred_pos: Vector2i, rng: RandomNumberGenerator) -> Room:
	...
	var room = Room.new(id, type, preferred_pos, size)
...
func _try_place_room(id: int, type: RoomType, existing_rooms: Array[Room], rng: RandomNumberGenerator) -> Room:
...
func _room_overlaps(room: Room, existing_rooms: Array[Room]) -> bool:
...
func _connect_rooms(rng: RandomNumberGenerator) -> bool:
	...
	if generated_corridors.is_empty():
		print("DungeonWorld: Failed to generate corridors")
		return false
	...
	print("DungeonWorld: Connected %d rooms with %d corridors" % [rooms.size(), corridors.size()])
	return true
...
func _generate_dungeon_geometry() -> bool:
	...
	if not tilemap_layer or not tilemap_layer.tile_set:
		print("DungeonWorld: Cannot generate geometry - TileMap not properly configured")
		return false
	
	tilemap_layer.clear()
	
	# Configurar generadores con la misma semilla
	room_generator.set_seed(randi())
	
	# Generar habitaciones usando el RoomGenerator profesional
	for room in rooms:
		if not room_generator.generate_room_layout(room, tilemap_layer):
			print("DungeonWorld: Failed to generate room %d" % room.id)
			return false
...
func _draw_corridor(room_a: Room, room_b: Room):
...
func _find_door_position(room_a: Room, room_b: Room) -> Vector2i:
...
func _populate_single_room(room: Room, rng: RandomNumberGenerator):
...
func _place_player_spawn(room: Room):
	...
	print("DungeonWorld: Player spawn set at %s" % spawn_pos)

func _place_enemies(room: Room, count: int, rng: RandomNumberGenerator):
	"""Coloca enemigos en una habitación"""
	for i in range(count):
		var enemy_pos = _get_random_room_position(room, rng)
		# TODO: Spawn enemy at enemy_pos
		print("DungeonWorld: Would spawn enemy at %s in room %d" % [enemy_pos, room.id])

func _place_treasure(room: Room, rng: RandomNumberGenerator):
	"""Coloca tesoro en una habitación"""
	var treasure_pos = _get_random_room_position(room, rng)
	# TODO: Spawn treasure at treasure_pos
	print("DungeonWorld: Would spawn treasure at %s in room %d" % [treasure_pos, room.id])
...
func _place_boss(room, _rng):
	...
	print("DungeonWorld: Would spawn boss at %s in room %d" % [boss_pos, room.id])

func _place_secret_treasure(room: Room, rng: RandomNumberGenerator):
	"""Coloca tesoro especial en habitación secreta"""
	var treasure_pos = _get_random_room_position(room, rng)
	# TODO: Spawn special treasure
	print("DungeonWorld: Would spawn secret treasure at %s in room %d" % [treasure_pos, room.id])
...
func _place_shop_keeper(room, _rng):
	...
	print("DungeonWorld: Would spawn shop keeper at %s in room %d" % [keeper_pos, room.id])

func _place_shop_items(room: Room, rng: RandomNumberGenerator):
	"""Coloca objetos en la tienda"""
	var item_count = 3
	for i in range(item_count):
		var item_pos = _get_random_room_position(room, rng)
		# TODO: Spawn shop item
		print("DungeonWorld: Would spawn shop item at %s in room %d" % [item_pos, room.id])

func _get_random_room_position(room: Room, rng: RandomNumberGenerator) -> Vector2:
...
func open_door(door: Door) -> bool:
	...
	if door.state == DoorState.LOCKED:
		...
		print("DungeonWorld: Door %d opened" % door.id)
		return true
	return false

func complete_room(room: Room):
	...
	print("DungeonWorld: Room %d completed" % room.id)
...
func _check_dungeon_completion():
	...
	if current_dungeon.boss_room and current_dungeon.boss_room.is_completed:
		...
		print("DungeonWorld: Dungeon completed!")
